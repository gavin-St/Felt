...
Uses Felt's shared `baseline_100bb_v1` chart preflop, except that it calls every
hand from the big blind against a single 2 bb open. With top pair or an
...
value each time. On the river it always raises trips or better when facing a
bet, because there is no later street on which a trap can earn more.
