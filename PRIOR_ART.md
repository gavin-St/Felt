...
DeepStack's median 2.3 s per action is about **23,000x** our 100 µs cap. That gap is
...
That matters here: a 100 µs cap is not as brutal as the DeepStack figure suggests.
...
| Time per decision | seconds | 100 µs default |
...
1. **CPU-time measurement is impossible over a socket.** A socket protocol can
   ...
   means giving up the 100 µs CPU cap and reintroducing a time bank — reversing a
...
| Budget | 6 s **average per hand**, 10 s per-action ceiling, 10 min per-hand ceiling | **30 s per player for the whole match** | **100 µs per decision** |
| ≈ per decision | ~2 s | ~10 ms | 0.1 ms |
| Bot compute per match (both bots, worst case) | ~10 h | **60 s** | **12 s** |
...
**Felt gives less total bot compute than MIT** — roughly 12 s rather than 60 s —
with a per-decision cap about 100× tighter, despite spreading decisions across
