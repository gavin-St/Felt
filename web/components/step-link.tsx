 ...
 * appear. Tall and narrow, so the edge it occupies is easy to hit without the
 * shape covering anything; faint with the header, and close to solid only
 * under the pointer.
 *
...
export function StepLink({
  ...
  return (
    <Link
      ...
      } flex h-20 w-6 select-none items-center justify-center rounded-sm bg-[#231f1b] font-mono text-xl leading-none text-[#f6f2e9] opacity-0 transition-opacity hover:opacity-85 focus-visible:opacity-85 group-hover:opacity-35`}
    >
      {direction === 'previous' ? '‹' : '›'}
    </Link>
  );
