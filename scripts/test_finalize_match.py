...
class FinalizeMatchTest(unittest.TestCase):
    ...
    def test_rating_fit_prioritizes_wins_with_bounded_margin_and_field_scale(
        self,
    ) -> None:
        ...
        tiny_win = rebuild_ratings.direction_first_logit(0.001, 1.0)
        huge_win = rebuild_ratings.direction_first_logit(100.0, 1.0)
        self.assertGreater(tiny_win, rebuild_ratings.BASE_WIN_LOGIT)
        self.assertLess(
            huge_win,
            rebuild_ratings.BASE_WIN_LOGIT
            + rebuild_ratings.MARGIN_BONUS_LOGIT
            + 1e-9,
        )
